...
void tree<Key, T, Compare>::BalanceTree(Node* inserted) {
  ...
  while (parent != fake_node_ && parent->red) {
    Dir side = parent->Side();
    Dir opposite_side = ReverseSide(side);
    uncle = parent->parent->GetChild(opposite_side);
    if (uncle != fake_node_ && uncle->red) {
      ...
      if (parent->GetChild(opposite_side) == inserted) {
        parent = RotateDirRoot(parent, side)->parent;
      }
      parent->red = false;
      parent->parent->red = true;
      RotateDirRoot(parent->parent, opposite_side);
    }
    fake_node_->left->red = false;
  }
  fake_node_->left->red = false;
...
typename tree<Key, T, Compare>::Node* tree<Key, T, Compare>::RotateDirRoot(
  ...
  Dir opposite_side = ReverseSide(side);
  Node* right_node = node->GetChild(opposite_side);
  if (parent->left == node) {
    parent->SetChild(right_node, Dir::kLeft);
  } else {
    parent->SetChild(right_node, Dir::kRight);
  }
  right_node->parent = parent;
  node->SetChild(right_node->GetChild(side), opposite_side);
  right_node->GetChild(side)->parent = node;
  right_node->SetChild(node, side);
